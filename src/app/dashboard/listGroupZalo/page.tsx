...
import { FiUsers, FiLoader, FiAlertTriangle, FiSearch, FiSliders, FiShield, FiX, FiGrid, FiUserCheck, FiMessageSquare, FiSend, FiHelpCircle, FiChevronDown, FiEye, FiCheckCircle, FiUserPlus, FiLink } from 'react-icons/fi';
...
// ✨ THÊM MỚI: Popup xem thông tin nhóm bằng link
const ViewGroupByLinkModal = ({ onClose, onNavigate }: { onClose: () => void; onNavigate: (link: string) => void; }) => {
    ...
    const handleView = () => {
        ...
        if (match && match[0]) {
            onNavigate(link); // Gửi nguyên link lên
        } else {
...
const BulkSendMessageModal = ({ allGroups, selectedAccount, onSubmit, onClose }: { allGroups: ZaloGroup[]; selectedAccount: any; onSubmit: (message: string, groupIds: string[]) => void; onClose: () => void; }) => {
    ...
    }, [allGroups, searchTerm, minMembers, maxMembers, roleFilter, typeFilter, selectedAccount, showAdvanced]);

    const handleToggleSelect = (groupId: string) => {
        const newSelectedIds = new Set(selectedIds);
        newSelectedIds.has(groupId) ? newSelectedIds.delete(groupId) : newSelectedIds.add(groupId);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl flex flex-col h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-grow flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
                    {/* ✨ SỬA LỖI: Cải thiện layout mobile */}
                    <div className="w-full md:w-2/5 border-b md:border-b-0 md:border-r border-gray-700 p-4 flex flex-col space-y-3 overflow-hidden h-1/2 md:h-auto">
                        <div className="flex-shrink-0 space-y-3">
                            <div className="relative"><FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input type="text" placeholder="Tìm kiếm nhóm theo tên..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-gray-700 text-white pl-10 pr-4 py-2 rounded-md border border-gray-600"/></div>
                            ...
                            {showAdvanced && (
                                <div className="space-y-3 p-3 bg-gray-900/50 rounded-md animate-fade-in-down">
                                    <div className="flex items-center gap-2"><input type="number" placeholder="Tối thiểu TV" value={minMembers} onChange={e => setMinMembers(e.target.value)} className="w-1/2 bg-gray-700 text-white p-2 rounded-md text-sm" /><span>-</span><input type="number" placeholder="Tối đa TV" value={maxMembers} onChange={e => setMaxMembers(e.target.value)} className="w-1/2 bg-gray-700 text-white p-2 rounded-md text-sm" /></div>
                                    <select value={roleFilter} onChange={e => setRoleFilter(e.target.value as any)} className="w-full bg-gray-700 text-white p-2 rounded-md text-sm"><option value="all">Tất cả vai trò</option><option value="admin">Nhóm tôi quản lý</option><option value="member">Nhóm tôi tham gia</option></select>
                                    <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as any)} className="w-full bg-gray-700 text-white p-2 rounded-md text-sm"><option value="all">Tất cả loại nhóm</option><option value="community">Nhóm cộng đồng</option><option value="normal">Nhóm thường</option></select>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex-grow space-y-2 overflow-y-auto pr-2">
                            {filteredList.map(group => (
                                <label key={group.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-700 cursor-pointer"><input type="checkbox" checked={selectedIds.has(group.id)} onChange={() => handleToggleSelect(group.id)} className="form-checkbox h-5 w-5 bg-gray-900 border-gray-600 text-blue-500"/><Image src={group.avatar || '/avatar-default-crm.png'} alt={group.name} width={40} height={40} className="rounded-full" onError={(e) => { e.currentTarget.src = '/avatar-default-crm.png'; }}/><span className="text-white truncate">{group.name}</span></label>
                            ))}
                        </div>
                    </div>
...
export default function ListGroupZaloPage() {
    ...
    const groupStats = useMemo(() => {
        if (!isClient || !selectedAccount) return { total: 0, admin: 0, member: 0 };
        const adminCount = groups.filter(g => Array.isArray(g.admins) && g.admins.includes(selectedAccount.profile.userId)).length;
        return { total: groups.length, admin: adminCount, member: groups.length - adminCount };
    }, [groups, selectedAccount, isClient]);

    const filteredGroups = useMemo(() => {
        if (!isClient || !selectedAccount) return [];
        return groups
            .filter(group => {
                if (searchTerm && group.name && !group.name.toLowerCase().includes(searchTerm.toLowerCase())) return false;
                const min = parseInt(minMembers, 10);
                const max = parseInt(maxMembers, 10);
                if (!isNaN(min) && group.totalMembers < min) return false;
                if (!isNaN(max) && group.totalMembers > max) return false;
    ...
    const handleNavigateToGroupDetails = (identifier: string) => {
        // Mã hóa link để truyền an toàn qua URL
        const encodedIdentifier = encodeURIComponent(identifier);
        router.push(`/dashboard/group-details/${encodedIdentifier}`);
    };

    const handleBulkSendSubmit = async (message: string, recipientIds: string[]) => {
        if (!selectedAccount || recipientIds.length === 0) {
            alert("Vui lòng chọn tài khoản và ít nhất một nhóm để gửi.");
            return;
        }
    ...
    const renderContent = () => {
        ...
        if (!selectedAccount) { return <div className="text-center text-yellow-400 mt-10"><FiAlertTriangle className="mx-auto h-12 w-12" /><h3 className="mt-2 text-xl font-semibold">Chưa chọn tài khoản</h3><p>Vui lòng chọn một tài khoản Zalo từ menu ở trên header.</p></div>; }
        if (error) { return <div className="text-center text-red-400 mt-10 p-4 bg-red-500/10 rounded-md">{error}</div>; }
        if (filteredGroups.length === 0 && groups.length > 0) { return <div className="text-center text-gray-400 mt-10">Không tìm thấy nhóm nào khớp với bộ lọc.</div>; }
        if (groups.length === 0) { return <div className="text-center text-gray-400 mt-10">Tài khoản này chưa tham gia nhóm nào.</div>; }
        ...
        return (
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {filteredGroups.map(group => {
                    const isAdmin = Array.isArray(group.admins) && group.admins.includes(selectedAccount.profile.userId);
                    return (
                        <Link href={`/dashboard/group-details/${group.id}`} key={group.id}>
                            <div className="bg-gray-800 p-4 rounded-lg flex flex-col items-center text-center transition-all hover:bg-gray-700 hover:-translate-y-1 h-full">
                                <div className="relative">
                                    <Image src={group.avatar || '/avatar-default-crm.png'} alt={group.name} width={64} height={64} className="rounded-full mb-3" onError={(e) => { e.currentTarget.src = '/avatar-default-crm.png'; }}/>
                                    {isAdmin && (<div className="absolute bottom-2 right-0 bg-yellow-500 p-1 rounded-full border-2 border-gray-800" title="Bạn là quản trị viên"><FiShield size={10} className="text-white"/></div>)}
                                </div>
                                <p className="text-sm font-semibold text-white truncate w-full flex-grow">{group.name}</p>
                                <p className="text-xs text-gray-400 mt-1">{group.totalMembers} thành viên</p>
                            </div>
                        </Link>
                    )
                })}
    ...
    return (
        <div className="flex-1 p-6 md:p-8">
            ...
            {isClient && isBulkSendModalOpen && <BulkSendMessageModal allGroups={groups} selectedAccount={selectedAccount} onClose={() => setIsBulkSendModalOpen(false)} onSubmit={handleBulkSendSubmit} />}
            {successInfo && (
                <SuccessModal 
                    count={successInfo.count}
                    onClose={() => setSuccessInfo(null)}
                    onViewProgress={() => router.push('/dashboard/listSendMessageStranger')}
                />
            )}

            <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
                <h1 className="text-3xl font-bold text-white flex items-center gap-3"><FiUsers />Danh sách nhóm</h1>
            ...
            {isClient && selectedAccount && !loading && groups.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <StatsCard icon={<FiGrid size={24} className="text-blue-400" />} title="Tổng số nhóm" value={groupStats.total} color="#3b82f6" />
                    <StatsCard icon={<FiShield size={24} className="text-yellow-400" />} title="Nhóm quản lý" value={groupStats.admin} color="#f59e0b" />
                    <StatsCard icon={<FiUserCheck size={24} className="text-green-400" />} title="Nhóm tham gia" value={groupStats.member} color="#10b981" />
                </div>
            )}

            {isClient && selectedAccount && !loading && (
                <div className="space-y-4 mb-6">
                    <div className="flex items-center bg-gray-800 border border-gray-700 rounded-md focus-within:ring-2 focus-within:ring-blue-500">
                        <FiSearch className="text-gray-400 mx-4" />
                        <input type="text" placeholder={`Tìm trong ${groups.length} nhóm...`} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-transparent text-white py-3 pr-4 focus:outline-none"/>
                    </div>
                    {showFilters && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-gray-800 rounded-md border border-gray-700 animate-fade-in-down">
                            <div className="flex items-center gap-2">
                                <input type="number" placeholder="Tối thiểu" value={minMembers} onChange={e => setMinMembers(e.target.value)} className="w-1/2 bg-gray-700 text-white p-2 rounded-md text-sm" />
                                <span>-</span>
                                <input type="number" placeholder="Tối đa" value={maxMembers} onChange={e => setMaxMembers(e.target.value)} className="w-1/2 bg-gray-700 text-white p-2 rounded-md text-sm" />
                            </div>
                            <div><select value={roleFilter} onChange={e => setRoleFilter(e.target.value as any)} className="w-full bg-gray-700 text-white p-2 rounded-md text-sm"><option value="all">Tất cả vai trò</option><option value="admin">Nhóm tôi quản lý</option><option value="member">Nhóm tôi tham gia</option></select></div>
                            <div><select value={typeFilter} onChange={e => setTypeFilter(e.target.value as any)} className="w-full bg-gray-700 text-white p-2 rounded-md text-sm"><option value="all">Tất cả loại nhóm</option><option value="community">Nhóm cộng đồng</option><option value="normal">Nhóm thường</option></select></div>
                             <button onClick={() => { setSearchTerm(''); setMinMembers(''); setMaxMembers(''); setRoleFilter('all'); setTypeFilter('all'); }} className="bg-gray-600 hover:bg-gray-500 text-white text-sm rounded-md">Xóa bộ lọc</button>
                        </div>
                    )}
                </div>
            )}
