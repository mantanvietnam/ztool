...
import { FiUsers, FiClock, FiPauseCircle, FiPlayCircle, FiPlus, FiLoader, FiCheckCircle, FiX, FiChevronLeft, FiChevronRight, FiXCircle, FiBarChart2, FiSearch } from 'react-icons/fi';
...
interface AddMemberJob {
    ...
    group_name?: string;
    group_total_members?: number; // ✨ THÊM MỚI: Số lượng thành viên nhóm
}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-4 overflow-y-auto">
                    <div className="text-sm"><span className="font-semibold text-gray-400 w-40 inline-block">Nhóm:</span><span className="text-gray-200 font-bold">{job.group_name || 'Không rõ'}</span></div>
                    {/* ✨ THÊM MỚI: Hiển thị số lượng thành viên */}
                    <div className="text-sm"><span className="font-semibold text-gray-400 w-40 inline-block">Thành viên:</span><span className="text-gray-200">{job.group_total_members?.toLocaleString() || 'N/A'}</span></div>
                    ...
                    <div><h4 className="font-semibold text-green-400 mb-2">Thêm thành công ({job.list_done.length})</h4><textarea readOnly value={job.list_done.join('\n')} rows={5} className="w-full bg-gray-900 text-green-300 text-sm p-2 rounded-md border border-gray-600"/></div>
                    <div><h4 className="font-semibold text-red-400 mb-2">Thêm lỗi ({job.list_error.length})</h4><textarea readOnly value={job.list_error.join('\n')} rows={5} className="w-full bg-gray-900 text-red-400 text-sm p-2 rounded-md border border-gray-600"/></div>
                </div>
...
const AddMemberToGroupModal = ({ onClose, onSubmit }: { onClose: () => void; onSubmit: (groupId: string, phones: string[]) => Promise<void>; }) => {
    ...
    const handleSubmit = async () => {
        if (!selectedGroupId || !phoneList.trim() || isSubmitting) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl flex flex-col h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 bg-gray-900 flex justify-end">
                    <button onClick={handleSubmit} disabled={isSubmitting || isLoadingGroups} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md transition duration-300 disabled:opacity-50">
...
export default function ListRequestAddMemberGroupPage() {
    ...
    const fetchData = useCallback(async (page: number) => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/apis/getListRequestAddMemberToGroupAPI`, {
                token: token,
                page: page,
                limit: limit,
            ...
            if (data.code === 0) {
                ...
                if (initialJobs.length > 0) {
                    ...
                    const groupInfoPromises = groupIds.map(groupId => 
                        axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get-group-info/${groupId}`, {
                    ...
                    groupInfoResults.forEach(result => {
                        if (result && result.data.success) {
                            const groupInfo = result.data.details.groupInfo;
                            groupInfoMap.set(groupInfo.groupId, { name: groupInfo.name, totalMembers: groupInfo.totalMember });
                        }
                    });

                    const jobsWithGroupNames = initialJobs.map((job: AddMemberJob) => ({
                        ...job,
                        group_name: groupInfoMap.get(job.group_id)?.name || 'Không tìm thấy nhóm',
                        group_total_members: groupInfoMap.get(job.group_id)?.totalMembers
                    }));
    ...
    const handleAddMemberSubmit = async (groupId: string, phoneNumbers: string[]) => {
        ...
        if (!selectedAccount) {
            throw new Error("Vui lòng chọn một tài khoản Zalo để thực hiện.");
        }
        try {
    ...
    const handleUpdateStatus = async (jobId: string | number, status: 'cancel' | 'pause' | 'process') => {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/apis/updateStatusRequestAddMemberToGroupAPI`, {
                token: token,
                id: jobId,
                status: status
            });
    ...
    const renderMainContent = () => {
        ...
        return (
            <>
                        ...
                        <table className="w-full text-sm text-left text-gray-300">
                            <thead className="text-xs text-gray-400 uppercase bg-gray-700">
                                <tr>
                                    ...
                                    {/* ✨ THÊM MỚI: Cột thành viên */}
                                    <th scope="col" className="px-6 py-3">Thành viên</th>
                                    <th scope="col" className="px-6 py-3">Đã thêm / Tổng số</th>
                            ...
                            <tbody>
                                {jobs.map((job) => (
                                    <tr key={job.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                                        ...
                                        <td className="px-6 py-4 font-semibold">{job.group_name || job.group_id}</td>
                                        {/* ✨ THÊM MỚI: Hiển thị số lượng thành viên */}
                                        <td className="px-6 py-4">{job.group_total_members?.toLocaleString() ?? 'N/A'}</td>
                ...
                <Pagination currentPage={pagination.currentPage} totalPages={pagination.totalPages} basePath="/dashboard/listRequestAddMemberGroup" />
            </>
        );
    };
    ...
    return (
        <div className="flex-1 p-6 md:p-8">
            ...
            {isAddModalOpen && <AddMemberToGroupModal onClose={() => setIsAddModalOpen(false)} onSubmit={handleAddMemberSubmit} />}
            
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-white">Yêu cầu thêm thành viên vào nhóm</h1>
                <button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md">
                    <FiPlus /> Tạo mới
                </button>
            </div>
