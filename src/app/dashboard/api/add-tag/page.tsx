...
// --- MAIN PAGE ---
export default function ApiAddTagDocPage() {
    const { selectedAccount } = useZaloAccounts();
    const [token, setToken] = useState('');
    
    // Đổi endpoint sang API thêm thành viên vào thẻ (dựa theo logic tag_detail)
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/apis/saveMemberTagAPI`;
    ...
    // Mẫu Body JSON (Mô phỏng mảng object member như trong tag_detail.tsx)
    const exampleBody = `{
    "token": "${token || 'YOUR_API_TOKEN'}",
    "userId": "${userId || 'USER_ID_ZALO'}",
    "tag_id": "ID_CUA_THE_PHAN_LOAI",
    "phones": "['081656xxx', '0369666yyy']"
}`;
...
--data '${exampleBody.replace(/\n/g, '')}'`;
    ...
    "code": 0,
    "mess": "Thêm thành công 2 thành viên vào thẻ!",
    "data": null
}`;

    const exampleResponseError = `{
    "code": 1, 
    "mess": "Thẻ không tồn tại hoặc tài khoản Zalo bị lỗi."
}`;
        ...
        <div className="flex-1 p-6 md:p-8 space-y-8 max-w-5xl mx-auto">
            ...
            <div className="border-b border-gray-700 pb-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h1 className="text-3xl font-bold text-white">API Gắn Thẻ Phân Loại</h1>
                </div>
                <p className="text-gray-400 mt-2">
                    Tài liệu hướng dẫn tích hợp API tự động thêm bạn bè hoặc khách hàng vào thẻ phân loại (Tag)
                </p>
            </div>
                    ...
                    <table className="w-full text-sm text-left text-gray-300">
                        ...
                        <tbody>
                            <ParamRow 
                                ...
                                desc={<div><p>Token xác thực API do hệ thống cấp.</p><InlineCopy value={token} label="Token" truncate={true} /></div>} 
                            />
                            <ParamRow 
                                name="userId" type="String" required={true} 
                                desc={<div><p>ID tài khoản Zalo sở hữu thẻ phân loại.</p>{userId ? <InlineCopy value={userId} label="UserId" /> : <span className="text-xs text-yellow-500 italic">Vui lòng chọn tài khoản Zalo</span>}</div>} 
                            />
                            <ParamRow 
                                name="tag_id" type="String / Number" required={true} 
                                desc="ID của thẻ phân loại trên Zalo. Bạn có thể xem ID này ở trang danh sách thẻ phân loại." 
                            />
                            <ParamRow 
                                name="phones" type="JSON Array" required={true} 
                                desc={<div><p>Chuỗi json chứa danh sách số điện thoại đối tượng người dùng cần gắn thẻ, chú ý sử dụng nháy kép.</p><p className="text-xs text-gray-500 mt-1">VD: <code>["098...", "091..."]</code></p></div>}
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded-lg">
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-300 space-y-2">
                            <li><strong>Quan hệ bạn bè:</strong> Tài khoản được gắn thẻ thường phải là bạn bè hoặc người đã từng nhắn tin với <code>userId</code>.</li>
                            <li><strong>Giới hạn Zalo:</strong> Zalo có thể áp dụng giới hạn về số lượng thành viên tối đa trong một thẻ.</li>
                            <li><strong>Định dạng Payload:</strong> Đảm bảo mảng <code>phone</code> truyền đúng định dạng json theo chuẩn của hệ thống để tránh lỗi không nhận diện được dữ liệu.</li>
                        </ul>
                    </div>
                </div>
            </div>
